...
def build_backbone(lr_backbone = 1e-5,position_embedding = 'sine', hidden_dim = 256, multiscale = False, masks = False, backbone= 'resnet50', dilation = False, return_interm_indices = [0,1,2,3]):
    position_embedding = build_position_encoding(position_embedding = position_embedding, hidden_dim = hidden_dim)
    train_backbone = float(lr_backbone) > 0
    return_interm_layers = masks or multiscale
    backbone = Backbone(backbone, train_backbone, dilation, return_interm_indices, batch_norm=FrozenBatchNorm2d)
